...
class ISerializer {
public:
    virtual std::string ProcessElem(MatrixPtr<T> matrix, size_t row, size_t col,
                                    size_t min_str_len) const = 0;
...
public:
    std::string ProcessElem(MatrixPtr<T> matrix, size_t row, size_t col,
                            size_t min_str_len) const override {
...
template <typename T>
std::string BasicProcessing(MatrixPtr<T> matrix, size_t row, size_t col,
                            size_t min_size = 6) {
    std::string val = std::to_string((*matrix)[row, col]);
